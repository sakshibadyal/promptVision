...
const Payment = ({ user, setUser }) => {
  ...
  const confirmPayment = window.confirm(
    `Confirm dummy payment for ${planId} plan?`
  );
      ...
      <div className="flex-grow flex flex-col items-center pt-40 pb-24 px-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl w-full">
          ...
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`p-8 rounded-2xl border flex flex-col transition-all duration-300 ${plan.popular
            ...
            >
              ...
              <button
                ...
                className={`w-full py-3 font-bold rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed ${plan.popular
